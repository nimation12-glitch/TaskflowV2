...
export async function register() {
  ...
  if (isProduction && !process.env.AUTH_SECRET) {
    throw new Error(
      "Refusing to start: production requires AUTH_SECRET (session/OAuth cryptographic secret — see backend/README.md).",
    );
  }
  ...
  if (configured.length === 0) {
    console.warn("[auth] no OAuth providers configured — social sign-in buttons render a truthful disabled state (see backend/README.md)");
  } else {
